...
module.exports = {
 ...
 getInstructorCourses: (instructor_id) => {
  ...
  // Create a new course
  create: (courseData) => {
    const { course_name, instructor_name, price, syllabus, image_url } = courseData;
    const query = 'INSERT INTO course (course_name, instructor_name, price, syllabus, image_url) VALUES (?, ?, ?, ?, ?)';
    return new Promise((resolve, reject) => {
      db.query(query, [course_name, instructor_name, price, syllabus, image_url], (err, result) => {
        if (err) {
  ...
  update: (course_id, courseData) => {
    const { course_name, instructor_name, price, syllabus, image_url } = courseData;
    const query = 'UPDATE course SET course_name = ?, instructor_name = ?, price = ?, syllabus = ?, image_url = ? WHERE course_id = ?';
    return new Promise((resolve, reject) => {
      db.query(query, [course_name, instructor_name, price, syllabus, image_url, course_id], (err, result) => {
        if (err) {
